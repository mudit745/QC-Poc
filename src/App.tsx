...
function App() {
  ...
  const filteredRules = useMemo(() => {
    return mockBusinessRules.filter((rule: BusinessRule) => {
      ...
      const businessRuleMatch = filters.businessRule === 'All' || rule.description === filters.businessRule;
      return moduleMatch && statusMatch && businessRuleMatch;
    });
  }, [filters]);
  ...
  const currentRuleThreads = useMemo(() => {
    ...
    if (filters.threadStatus !== 'All') {
      filteredThreads = filteredThreads.filter(thread => thread.status === filters.threadStatus);
    }
    
    // Apply thread title filter
    if (filters.threadTitle !== 'All') {
      filteredThreads = filteredThreads.filter(thread => thread.title === filters.threadTitle);
    }
  ...
  const actionStatusOptions = ['Action Required', 'In Progress', 'Completed', 'On Hold', 'Verified', 'Skipped'];
